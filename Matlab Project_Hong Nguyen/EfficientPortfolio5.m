function [EffWts, EffRet,EffRisk] = EfficientPortfolio5(R,S,target,NumPort,AssBnd,Groups,GrpBnd)
...
if nargin < 6 || isempty(GrpBnd) || isempty(Groups)  
    
A       = [];
B       = [];

elseif isreal(Groups) && isreal(GrpBnd)
...
EffWts      = fmincon(@(w) PortRisk(w,S),repmat((1/N),N,1),A,B,Aeq,Beq,LowBnd,UppBnd);    
...
EffWts(:,j) = fmincon(@(w) PortRisk(w,S),repmat((1/N),N,1),A,B,Aeq,Beq,LowBnd,UppBnd);
...
MVPWts      = fmincon(@(w) PortRisk(w,S),repmat((1/N),N,1),A,B,Aeq,Beq,LowBnd,UppBnd);    
...
MaxWts      = fmincon(@(w) -PtfRet(w,R),repmat((1/N),N,1),A,B,Aeq,Beq,LowBnd,UppBnd);    
